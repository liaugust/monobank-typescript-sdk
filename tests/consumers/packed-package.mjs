...
writeFileSync(
  ...
  'import { AccountType, MonobankAcquiringClient, MonobankPersonalClient, MonobankPublicClient, verifyAcquiringWebhookSignature } from "@liaugust/monobank-sdk";\n' +
    ...
    'if (AccountType.Black !== "black" || !publicApi.bank || !publicApi.currency || !personal.client || !personal.statements || !personal.webhooks || !acquiring.invoices || !acquiring.merchant || !acquiring.statements || !acquiring.webhooks || typeof verifyAcquiringWebhookSignature !== "function") process.exitCode = 1;\n',
);
writeFileSync(
  ...
  'const { CashbackType, MonobankAcquiringClient, MonobankPersonalClient, MonobankPublicClient, verifyAcquiringWebhookSignature } = require("@liaugust/monobank-sdk");\n' +
    ...
    'const personal = new MonobankPersonalClient({ token: "personal" });\n' +
    "const publicApi = new MonobankPublicClient();\n" +
    'if (CashbackType.UAH !== "UAH" || !publicApi.bank || !publicApi.currency || !personal.client || !personal.statements || !personal.webhooks || !acquiring.invoices || !acquiring.merchant || !acquiring.statements || !acquiring.webhooks || typeof verifyAcquiringWebhookSignature !== "function") process.exitCode = 1;\n',
);
